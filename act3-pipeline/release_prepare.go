...
func (r *Release) Prepare(ctx context.Context,
	...
	exists, err := r.pipeline.Source.Exists(ctx, changelogPath)
	if err != nil {
		return nil, fmt.Errorf("generating changelog: %w", err)
	}
	if !exists {
		r.pipeline.Source = r.pipeline.Source.WithNewFile(changelogPath, "")
	}
...
func (r *Release) gitStatus(ctx context.Context) error {
	ctr := dag.Wolfi().
		...
		WithMountedDirectory("/work/src", r.pipeline.Source).
		With(func(c *dagger.Container) *dagger.Container {
			if r.pipeline.GitIgnore != nil {
				const gitIgnorePath = "/work/.gitignore"
				c = c.WithMountedFile(gitIgnorePath, r.pipeline.GitIgnore).
...
func (r *Release) version(ctx context.Context,
	...
	version, err := dag.GitCliff(r.pipeline.Source, dagger.GitCliffOpts{Container: base}).
...
func (r *Release) changelog(ctx context.Context,
	...
	return dag.GitCliff(r.pipeline.Source, dagger.GitCliffOpts{Container: base}).
		WithTag(version).
		WithStrip("footer").
		WithUnreleased().
...
func (r *Release) notes(ctx context.Context,
	...
	notes, err := dag.GitCliff(r.pipeline.Source, dagger.GitCliffOpts{Container: base}).
		WithTag(version).
		WithUnreleased().
		WithStrip("all").
		Run(dagger.GitCliffRunOpts{Args: args}).
...
func (r *Release) setHelmChartVersion(
	...
	updatedChart := dag.Wolfi().
		...
		WithMountedDirectory("/src", r.pipeline.Source).
		WithWorkdir("/src").
